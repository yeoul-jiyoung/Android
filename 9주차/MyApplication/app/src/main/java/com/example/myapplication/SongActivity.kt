...
//    private val songRef=database.getReference("songs") //songs 경로 참조
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.songActivityLikeImgIv.setOnClickListener {
            ...
            val likedDao = songDB.likedSongDao()
            val isCurrentlyLiked = likedDao.isLiked(userId, currentSong.id)

            setLike(isCurrentlyLiked)
        }

        // 화면 들어올 때 현재 곡 좋아요 상태 반영
        updateLikeIcon()
    }
    private fun updateLikeIcon() {
        val userId = getJwt()
        val song = songs[nowPos]
        val likedSongDao = songDB.likedSongDao()
...
//            }
    ...
    //DB의 setLike
    private fun setLike(currentIsLike: Boolean) {
        val userId = getJwt()
        val song = songs[nowPos]
        val likedSongDao=songDB.likedSongDao()
